 ...
 * 可领判定 = 邀约落地后进入 30 分钟窗口（hasLanding + remainingMs）；每日上限按维度核算。
 */
export function useShareClaim(dim?: () => ShareDim | undefined) {
  ...
  async function refresh() {
    ...
    if (!isLoggedIn()) {
      state.value = { ...DEFAULT_SHARE_STATE, canClaim: false, waitLanding: true }
      return
    }
    loading.value = true
    try {
      ...
      state.value = {
        ...
        hasLanding: d?.hasLanding !== undefined ? !!d.hasLanding : (d?.waitLanding !== undefined ? !d.waitLanding : false),
        waitLanding: !!d?.waitLanding,
      }
      // 不可领时定期轮询刷新（等待新的邀约落地 / 次日上限重置 / 进入可领窗口）
      if (!state.value.canClaim) {
...
export function shareRuleText(s: ShareClaimState) {
  ...
  return `每次分享得 ${s.points} 积分${daily > 0 ? `，每日最多 ${daily} 次` : ''}，好友通过分享注册后 ${interval} 分钟内可领`
}
...
export function shareReasonText(s: ShareClaimState) {
  ...
  if (s.waitLanding) return '分享出去等待好友注册'
  if (s.hasLanding && s.remainingMs <= 0) return '邀约落地已超过领取窗口，等待新的好友注册'
  return '登录后可领取'
}